...
;
;---------------------------------------------------------------------
;  SBC Firmware V5.2, 2-24-2018, by Daryl Rictor
;
;*********************************************************************       
...
;
Buffer         =     $0280             ; keybd input buffer (127 chrs max)
;               
; *************************************************************************
; kernal commands
; *************************************************************************
...
InputWait      jsr   Input_Chr         ; get a character
               ...
               cmp   #$09              ; TAB key
               beq   Tabkey            ;
               bra   InputWait         ; ignore other keys
Backspace      cpy   #$ff              ;
...
;               JSR   Output            ;
               RTS                     ;
;
;Input_chr      jmp   serial_input      ;
;Scan_input     jmp   serial_scan       ; 
;Output         jmp   serial_output     ;
;
...
Help_cmd       lda   #<HelpTxt         ;  lower byte - Menu of Commands
               ...
               sta   Addrptr+1         ;
               bra   Help_cmd3         ;
Help_cmd4      cmp   #$7e              ;  "~" (used for new line)
               beq   Help_cmd1         ;
               jsr   Output            ;
               bra   Help_cmd2         ;
...
HelpTxt        .byte "~Commands are :~"
               ...
               .byte "[HHHH]{.HHHH}{Return} - Hex dump address range (16 per line)~"
               ...
               .byte "[HHHH]{L}{Return} - List (disassemble) 20 program lines~"
               .byte "[HHHH]{.HHHH}{L}{Return} - Dissassemble range~"
               .byte "{HHHH.HHHH>HHHH{M}{Return} - Move range at 1st HHHH thru 2nd to 3rd HHHH~"
               .byte "[HHHH][ HHHH]{Q}{Return} - Text dump address(s)~"
               .byte "[HHHH]{.HHHH}{Q}{Return} - Text dump address range (16 per line)~"
               .byte "{S}[up to 32 text characters]{Return} - Set 32 byte LED message~"
...
AsmHelpTxt     .byte "~Commands are :~"
               ...
               .byte "[HHHH]{L}{Return} - List (disassemble) 20 program lines~"
               .byte "{Return} - Exit Assembler~"
               .byte "{?}{Return} - Print commands~~"
               .byte $00
;
Porttxt        .byte "65C02 Monitor v5.2 (2-24-18)"
               .byte  $0d, $0a
               .byte "with EhBASIC (c) Lee Davison" 
               .byte  $0d, $0a
               .byte "(Press ? for help)"
               .byte $00
;
